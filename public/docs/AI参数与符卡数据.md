...
| -------- | -------- |------|-------|------|------|
| 46.50    | 34.00    | 0.80 | 14.70 | 0.34 | 0.81 |
...
- 实际收率上限 = 相性修正 * 收率上限 * exp(-0.5 * min(0, (AI底力-难度) * 底力系数 + 2.5)) * 熟练度修正
- 收率 = (实际收率上限-最低收率) /(1 + exp(-变化率 * (AI底力 * 底力系数+AI熟练度 * (1-底力系数)-难度))) + 最低收率

最低收率是一个较小的数字，而且随难度增加逐渐减小至接近0。
